...
shows_map = {}   # key = show_name
show_order = []  # preserve insertion order
...
for row in reader:
    ...
    shows_map[name]['sessions'].append({
        ...
        'ticketSaleTime': row['ticketSaleTime'].strip(),
        'price': row['price'].strip(),
        'ticketLink': row['ticketLink'].strip(),
        'source': 'official'
    })

# Sort sessions within each show by showTime
for show in shows_map.values():
    show['sessions'].sort(key=lambda s: s['showTime'])

# Build list and assign auto IDs
shows = []
...
# Sort shows by nearest upcoming session time; shows with no future sessions go to end
